...
def get_overlayed_img(image, gradients):
    ...
    overlayed = np.tile(
        np.expand_dims(np.mean(transformed, axis=2).copy(), 2), (1, 1, 3)
    )
...
def plot_together(
    image, gradients, blurred_image, new_grads, save_path="outputs/test.png"
):
    """
    ...
    plt.title(f"Before: max at x={max_x}, y={max_y}")
    plt.subplot(1, 4, 3)
    plt.imshow(transform_cifar(blurred_image))
    plt.title("Blurred one part")
    plt.subplot(1, 4, 4)
